...
export default function Footer() {
  ...
  const forPatients = ["Find a Doctor", "Book Appointment", "Treatments", "Emergency 24x7", "Technology", "Patient Testimonials", "CPR", "Disclaimer", "Terms & Conditions", "Privacy Policy", "Refund & Cancellations", "Medicine Information", "Jaanta Hai Medanta"];
  const specialities = ["Cardiac Care", "Cancer Care", "Neurosciences", "Gastrosciences", "Orthopaedics", "Renal Care", "Liver Transplant", "Bone Marrow", "Transplant", "Lung Transplant", "Chest Surgery", "More +"];
  const ourHospitals = ["India Med Ranchi", "India Med Gurugram", "India Med Patna", "India Med Lucknow", "India Med Indore", "India Med Noida", "India Med Subhash Chowk", "India Med Cybercity", "India Med Defence Colony", "India Med Golf course"];
  const corporate = ["Help Desk", "About us", "Blogs", "Feedback", "Contact Us"];
    ...
    <footer className="relative bg-white pt-20 pb-10 w-full overflow-hidden">
      <div className="max-w-[1320px] mx-auto px-6 flex flex-col lg:flex-row gap-12 lg:gap-16">
        ...
        <div className="flex flex-col gap-8 w-full lg:w-[320px] items-center lg:items-start text-center lg:text-left">
          ...
          <p className="text-[#58595B] font-montserrat font-medium text-[15px] lg:text-[16px] leading-[24px]">
            From treatment planning to recovery, we support international patients at every step - transparent costs, trusted hospitals, and hassle-free medical travel to India.
          </p>
          ...
          {[ 
            ...
            <div key={idx} className="text-center sm:text-left">
              ...
              <ul className="flex flex-col gap-2">
                {col.data.map((item, i) => (
                  <li key={`${idx}-${i}`}>
                    <Link href="#" className="text-[#58595B] font-montserrat font-semibold text-[15px] lg:text-[17px] leading-[26px] lg:leading-[30px] hover:text-[#EE4423]">
                      {item}
                    </Link>
                  </li>
                ))}
      ...
      <div className="max-w-[1320px] mx-auto px-6 mt-16 pt-8 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div className="flex gap-8">
          <Link href="#" className="text-[#58595B] text-sm font-medium">Privacy Policy</Link>
          <Link href="#" className="text-[#58595B] text-sm font-medium">Terms & Conditions</Link>
        </div>
      </div>
    </footer>
  );
